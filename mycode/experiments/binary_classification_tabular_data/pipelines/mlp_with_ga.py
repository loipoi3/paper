...
def run_mlp_with_ga(x: np.ndarray, y: np.ndarray, args) -> None:
    """
    Runs an MLP with Genetic Algorithm for binary classification of tabular data.
    ...
    """
    print("MLP with GA for the problem of binary classification of tabular data:")
    ...
    hidden_layers = tuple(args.mlp_with_spm_hidden_layers) if args.mlp_with_spm_hidden_layers is not None else (
        x.shape[1], 10, 15, 20, 15, 10, 1)
    scale_for_mutation = args.mlp_with_spm_scale_for_mutation if args.mlp_with_spm_scale_for_mutation is not None else 0.5
