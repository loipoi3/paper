...
import numpy as np
import pandas as pd
from mycode.experiments.binary_classification_image_data.model import Model
from custom_dataset import ChestXRayDataset


def preprocess_labels(new_labels: np.ndarray) -> np.ndarray:
    """
    Preprocess labels to binary format.
    ...
    Returns:
        Binary labels.
    """
    return np.where(new_labels > 0, 1, 0)
...
if __name__ == "__main__":
    ...
    model = Model(3, True)
    checkpoint = torch.load('experiments/binary_classification_image_data/binary_classification_image_data_model.pth')
